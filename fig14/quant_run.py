...
for backend in backends:
    ...
    backend_folder = f"./results_cc_quant/{backend}"
    ...
    for run_id in range(1, num_runs + 1):
        ...
        for batch_size in batch_sizes:
            for input_length in input_lengths:
                # Base command
                base_command = f"nsys profile --trace=cuda,nvtx,osrt --cuda-memory-usage=true --output {run_folder}/{backend}-{batch_size}-{input_length} python quant.py --output-len 150"
