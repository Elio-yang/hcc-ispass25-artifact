...
int main(int argc,char* argv[]){
  ...
  FILE *fp;
  char buf[64];
  fp = fopen("/shared/uvm_bench/data/kmeans/1000000_points.txt", "r");
  ...
  fp = fopen("/shared/uvm_bench/data/kmeans/initCoord.txt", "r");
  while(fgets(buf, 64, fp) != NULL){
    k += 1;
  }
  fclose(fp);
  ...
  FILE * fp2 = fopen("/shared/uvm_bench/log/kmeans.txt", "a");
	if (fp2 == NULL) {
